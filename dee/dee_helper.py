...
class DEEExample(object):
    def __init__(self, annguid, detail_align_dict, only_inference=False):
        self.guid = annguid
        # [sent_text, ...]
        self.sentences = detail_align_dict['sentences']   # list[]
        self.prompt_sentences = detail_align_dict['prompt_sentences']   # list[]
        ...
        if only_inference:
            ...
            # [span_text, ...]
            self.ann_valid_mspans = detail_align_dict['ann_valid_mspans']   # ["300042", "朗科科技", "深圳市朗科科技股份有限公司"]
            # span_text -> [drange_tuple, ...]
            self.ann_mspan2dranges = detail_align_dict['ann_mspan2dranges']   # {"300042": [[0, 5, 11]], "朗科科技": [[0, 16, 20]], "深圳市朗科科技股份有限公司": [[1, 0, 13], [3, 0, 13], [14, 0, 13]]}
            # span_text -> guessed_field_name
            self.ann_mspan2guess_field = detail_align_dict['ann_mspan2guess_field']   # {"300042": "StockCode", "朗科科技": "StockAbbr", "深圳市朗科科技股份有限公司": "CompanyName"}
            # [(recguid, event_name, event_dict), ...]
            self.recguid_eventname_eventdict_list = detail_align_dict['recguid_eventname_eventdict_list']   # [[0, "EquityPledge", {"Pledger": "邓国顺", "PledgedShares": "1799900股", "Pledgee": "南京证券股份有限公司", "TotalHoldingShares": "28900000股", "TotalHoldingRatio": "21.63%", "TotalPledgedShares": "22339900股", "StartDate": "2017年5月4日", "EndDate": "2019年5月3日", "ReleasedDate": null}]]
            ...
            for sent_idx in range(self.num_sentences):
                self.sent_idx2srange_mspan_mtype_tuples[sent_idx] = []   # {0:[],1:[]}

            for mspan in self.ann_valid_mspans:   # "300042"
                for drange in self.ann_mspan2dranges[mspan]:   # [0, 5, 11]
                    sent_idx, char_s, char_e = drange
                    sent_mrange = (char_s, char_e)   # (5, 11)

                    sent_text = self.sentences[sent_idx]   # 第sent_idx句话
                    ...
                    guess_field = self.ann_mspan2guess_field[mspan]  # 角色StockCode

                    self.sent_idx2srange_mspan_mtype_tuples[sent_idx].append(
                        (sent_mrange, mspan, guess_field)   # ((5, 11),"300042",StockCode)
                    )
            ...
            for mrecguid, event_name, event_dict in self.recguid_eventname_eventdict_list:
                event_class = event_type2event_class[event_name]   # EquityPledge
                event_obj = event_class()   # 类
                assert isinstance(event_obj, BaseEvent)   # event_obj是BaseEvent的子类
    ...
    def __repr__(self):
        ...
        if not self.only_inference:   # True
            dee_str += '  span info: (\n'
    ...
    def get_entity_label_list():
        visit_set = set()
        entity_label_list = [NERExample.basic_entity_label]   # ['O']

        for field in common_fields:   # common_fields = ['StockCode', 'StockAbbr', 'CompanyName']
            if field not in visit_set:
                visit_set.add(field)
                entity_label_list.extend(['B-' + field, 'I-' + field])   # ['O','B-o','I-o']
...
class DEEExampleLoader(object):
    ...
    def rearrange_sent_info(self, detail_align_info):
        ...
        raw_dranges = detail_align_info['ann_valid_dranges']   # [[0, 5, 11], [0, 16, 20], [1, 0, 13]]
...
class DEEFeature(object):
    def __init__(self, guid, ex_idx, doc_token_id_mat, doc_token_mask_mat, doc_token_label_mat,
                 ...
                 valid_sent_num=None, prompt_token_id_mat=None, prompt_token_mask_mat=None, prompt_segment_ids_mat=None):
        self.guid = guid
        ...
        self.prompt_token_ids = torch.tensor(prompt_token_id_mat, dtype=torch.long)
        self.prompt_token_masks = torch.tensor(prompt_token_mask_mat, dtype=torch.uint8)  # uint8 for mask
        self.prompt_token_segment = torch.tensor(prompt_segment_ids_mat, dtype=torch.long)

        # sorted by the first drange tuple
        # [(token_id, ...), ...]
        # span_idx -> span_token_id tuple每个token对应的
        self.span_token_ids_list = span_token_ids_list
        # [[(sent_idx, char_s, char_e), ...], ...]
        # span_idx -> [drange tuple, ...]按句子排序
        self.span_dranges_list = span_dranges_list

        # [event_type_label, ...]
        # length = the total number of events to be considered
        # event_type_label \in {0, 1}, 0: no 1: yes事件类型标签
    ...
    def generate_dag_info_for(self, pred_span_token_tup_list, return_miss=False):
        ...
        pred_event_arg_idxs_objs_list = []#事件类型标签
        ...
        #pred_dag_info[None, None, None, [{(): {2}}, {(2,): {4}}, {(2, 4): {3}}, {(2, 4, 3): {3}}, {(2, 4, 3, 3): {5}}, {(2, 4, 3, 3, 5): {None}}], None]
        #所有前置序列
        else:
    ...
    def build_key_event_sent_info(self):
        ...
        for key_sent_idx_set, event_label, event_arg_idxs_objs in zip(
            ...
            if event_label == 0:
                ...
                for event_arg_idxs_obj in event_arg_idxs_objs:#同一个事件类型可能有多个事件
                    sent_idx_cands = []
                    ...
                    key_sent_idx_set.add(key_sent_idx)#找论元所在的关键句的id
        ...
        for sent_idx in range(self.valid_sent_num):  # masked sents will be truncated at the model part
            ...
            doc_sent_labels.append(sent_labels)#事件对应的关键句，句子与第几个事件类型有关
...
class DEEFeatureConverter(object):
    def __init__(self, entity_label_list, event_type_fields_pairs,
        ...
        # prepare entity_label -> entity_index mapping，实体类别标签映射
        ...
        # prepare event_type -> event_index and event_index -> event_fields mapping，事件类别标签映射
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        prompt_token_id_mat = []  # [[token_idx, ...], ...]
        prompt_token_mask_mat = []  # [[token_mask, ...], ...]
        prompt_segment_ids_mat = []

        for sent_idx, (sent_text, prompt_text) in enumerate(zip(dee_example.sentences, dee_example.prompt_sentences)):
            ...
            valid_token_len = self.max_sent_len
            if self.include_cls:
                valid_token_len -= 1
            if self.include_sep:
                valid_token_len -= 1
            prompt_text = self.tokenizer.char_tokenize(prompt_text)
            # truncate according to max_seq_len and record some statistics
            if len(prompt_text) > valid_token_len:  # 文本和标签的截断处理
                prompt_text = prompt_text[:valid_token_len]
            ...
            prompt_input_token_ids = self.tokenizer.convert_tokens_to_ids(fea_tokens)

            fea_seq_len = len(prompt_input_token_ids)
            fea_segment_ids = [0] * fea_seq_len
            fea_masks = [1] * fea_seq_len

            # feature is padded to max_seq_len, but fea_seq_len is the real length
            while len(prompt_input_token_ids) < self.max_sent_len:
                prompt_input_token_ids.append(0)
                fea_masks.append(0)
                fea_segment_ids.append(0)
            #assert prompt_input_token==ner_feature.input_ids

            prompt_token_id_mat.append(prompt_input_token_ids)
            prompt_token_mask_mat.append(fea_masks)
            prompt_segment_ids_mat.append(fea_segment_ids)

        assert len(doc_token_id_mat) == len(doc_token_mask_mat) == len(doc_token_label_mat) ==len(prompt_token_id_mat) == len(prompt_token_mask_mat) == len(prompt_segment_ids_mat) <= self.max_sent_num
        ...
        for mspan in dee_example.ann_valid_mspans:
            ...
            for sent_idx, char_s, char_e in raw_dranges:
                if char_base_s + char_e <= char_max_end and sent_idx < self.max_sent_num:
                    span_dranges.append((sent_idx, char_base_s + char_s, char_base_s + char_e))#加了一个cls的位置
                else:
            ...
            mspan2span_idx[mspan] = len(span_token_ids_list)#有效跨度转id
            span_token_ids_list.append(span_token_ids)#有效跨度可能出现多次
        ...
        for event_idx, event_type in enumerate(self.event_type_list):
            ...
            if event_type not in dee_example.event_type2event_objs:
                ...
                for event_obj in event_objs:
                    ...
                    for field in event_fields:#事件中的9个角色不一定都有，没有对应的为none。
                ...
                if event_arg_idxs_objs:
                    event_type_labels.append(1)#事件类型和角色。
        ...
        dee_feature = DEEFeature(
            ...
            prompt_token_id_mat=prompt_token_id_mat, prompt_token_mask_mat=prompt_token_mask_mat, prompt_segment_ids_mat=prompt_segment_ids_mat
        )
...
def prepare_doc_batch_dict(doc_fea_list):
    doc_batch_keys = ['ex_idx', 'doc_token_ids', 'doc_token_masks', 'prompt_token_ids', 'prompt_token_masks', 'doc_token_labels', 'valid_sent_num']
    doc_batch_dict = {}
