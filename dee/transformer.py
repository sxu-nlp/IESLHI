...
class EncoderLayer2(nn.Module):
    ...
    def forward(self, x, y, mask, **kwargs):
        """Follow Figure 1 (left) for connections."""
        x = self.sublayer[0](x, lambda x: self.self_attn(x, y, x, mask, **kwargs))
        return self.sublayer[1](x, self.feed_forward)
...
def make_transformer_encoder2(num_layers, hidden_size, ff_size=2048, num_att_heads=8, dropout=0.1,
                             raat=False, entity_structure="biaffine", num_structural_dependencies=21):
    dcopy = copy.deepcopy
    if raat:
        mh_att = MultiHeadAttentionRAAT(num_att_heads, hidden_size, entity_structure,
    ...
    tranformer_encoder = Encoder2(
        EncoderLayer2(hidden_size, dcopy(mh_att), dcopy(pos_ff), dropout=dropout),
        num_layers
    )
