...
def random_mask(span_token_tup_list, span_dranges_list, span_mention_range_list, mention_drange_list, mention_type_list, alpha_list):
    span_token_tup_list1 = []
    span_dranges_list1 = []
    span_mention_range_list1 = []
    mention_drange_list1 = []
    mention_type_list1 = []
    #randoms = 0.72
    print(random)
    span_dranges_list1x=[]
    for i in span_dranges_list:
        num = 0
        span_dranges_list1_=[]
        for j in i:
            number, _, _= j
            if number in alpha_list:
                span_dranges_list1_.append(j)
                num+=1
        if len(span_dranges_list1_)!=0:
            span_dranges_list1x.append(span_dranges_list1_)
        else:
            span_dranges_list1x.append([])
    for index_token, i in enumerate(span_dranges_list1x):
        if len(i)!=0:
            span_token_tup_list1.append(span_token_tup_list[index_token])
                # span_mention_range_list1.append()
                # mention_drange_list1.append(mention_drange_list[number])
                # mention_type_list1.append(mention_drange_list[number])
    for index_token, i in enumerate(span_dranges_list1x):
        if len(i)!=0:
            span_dranges_list1.append(span_dranges_list1x[index_token])
    start=0
    end=1
    for i in span_dranges_list1:
        span_mention_range_list1.append((start,end))
        start+=len(i)
        end+=len(i)
    count=0
    for i in range(len(mention_drange_list)):
        number, _, _ = mention_drange_list[i]
        if number in alpha_list:
            mention_drange_list1.append(mention_drange_list[i])
            mention_type_list1.append(mention_type_list[i])
        count+=1
    # for number, (sent_num, star, end) in enumerate(mention_drange_list):
    #     if randoms < alpha_list[number]:
    #
    #         span_token_tup_list1.append()
    #         span_dranges_list1.append()
    #         span_mention_range_list1.append()
    #         mention_drange_list1.append(mention_drange_list[number])
    #         mention_type_list1.append(mention_drange_list[number])
    #     else:
    #         span_token_tup_list = span_token_tup_list
    #         span_dranges_list = span_dranges_list
    #         span_mention_range_list = span_mention_range_list
    #         mention_drange_list = mention_drange_list
    #         mention_type_list = mention_type_list
    if len(span_token_tup_list1)==0:
        span_token_tup_list1, span_dranges_list1, span_mention_range_list1, mention_drange_list1, mention_type_list1 = span_token_tup_list, span_dranges_list, span_mention_range_list, mention_drange_list, mention_type_list
    return span_token_tup_list1, span_dranges_list1, span_mention_range_list1, mention_drange_list1, mention_type_list1

def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False, alpha_list=None):
    assert len(doc_token_types_list) == len(doc_fea_list)
    doc_span_info_list = []
    for doc_token_types, doc_fea, alpha in zip(doc_token_types_list, doc_fea_list, alpha_list):
        ...
        # one span may have multiple mentions同一实体的多次提及，范围
        ...
        if not use_gold_span:
            span_token_tup_list, span_dranges_list, span_mention_range_list, mention_drange_list, mention_type_list = random_mask(span_token_tup_list, span_dranges_list, span_mention_range_list, mention_drange_list, mention_type_list, alpha)
...
class ReDEEModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, id_2_vocab=None, ner_model=None,
        ...
        self.entity_dict = self.config.entity_dict
        self.role_dict = self.config.role_dict
        ...
        self.event_tables2 = nn.ModuleList([
            EventTable2(event_type, field_types, self.entity_dict, self.role_dict, config.hidden_size)
            for event_type, field_types in self.event_type_fields_pairs
        ])

        # self.event_tables3 = nn.ModuleList([
        #     EventTable3(event_type, field_types, config.hidden_size)
        #     for event_type, field_types in self.event_type_fields_pairs
        # ])
        self.event_cls_class_weight = nn.Linear(config.hidden_size, 2)
        ...
        self.doc_prompt_context_encoder = transformer.make_transformer_encoder2(
    ...
    def get_batch_sent_emb(self, ner_token_emb, ner_token_masks, valid_sent_num_list):
        ...
        if self.config.seq_reduce_type == 'AWA':
            ...
            total_sent_emb = ner_token_emb.max(dim=1)[0]#[33,768]
        elif self.config.seq_reduce_type == 'MeanPooling':
        ...
        total_sent_emb = self.sent_pos_encoder(total_sent_emb, sent_pos_ids=total_sent_pos_ids)#位置编码
    ...
    def get_event_cls_info2(self, doc_token_emb, doc_sent_context, doc_fea, prompt_sen_emb_list, doc_prompt_sent_list, entity_seq_cls,
                        role_seq_clss, train_flag=True):
        doc_event_sent_logps = []
        doc_event_prompt_logps = []
        doc_event_sent_prompt_logps = []

        for event_idx, event_label in enumerate(doc_fea.event_type_labels):
            #利用事件二分类标签作为句子权重
            sent_event_table = self.event_tables2[event_idx]
            event_query = entity_seq_cls[event_idx]
            cur_event_sent_logp, alpha_sent = sent_event_table(query_context_emb=event_query,
                                             sent_context_emb=doc_sent_context)  # [1, hidden_size]
            # 提示文本
            prompt_event_table = self.event_tables2[event_idx]
            cur_event_prompt_logp, atten_prompt = prompt_event_table(query_context_emb=event_query,
                                                 sent_context_emb=prompt_sen_emb_list)
            # 交互文档
            doc_event_table = self.event_tables2[event_idx]
            cur_event_prompt_sent_logp, atten_doc = doc_event_table(query_context_emb=event_query,
                                           sent_context_emb=doc_prompt_sent_list)

            doc_event_sent_logps.append(cur_event_sent_logp)
            doc_event_prompt_logps.append(cur_event_prompt_logp)
            doc_event_sent_prompt_logps.append(cur_event_prompt_sent_logp)

        # doc_event_class_logps = torch.cat(doc_event_class_weight_logps, dim=0)  # [num_event_types, 2]
        # doc_event_atten_logps = torch.cat(doc_event_atten_weight_logps, dim=0)  # [num_event_types, 2]
        doc_event_sent_logps = torch.cat(doc_event_sent_logps, dim=0)  # [num_event_types, 2]
        doc_event_prompt_logps = torch.cat(doc_event_prompt_logps, dim=0)  # [num_event_types, 2]
        doc_event_sent_prompt_logps = torch.cat(doc_event_sent_prompt_logps, dim=0)  # [num_event_types, 2]

        # doc_event_logps = torch.sum(doc_event_split_logps, dim=0)
        # doc_event_direct_logps = torch.cat(doc_event_direct_logps, dim=0)  # [num_event_types, 2]
        boot_pred_logp = doc_event_sent_logps+doc_event_sent_prompt_logps+doc_event_prompt_logps

        if train_flag:
            device = doc_event_prompt_logps.device
            doc_event_labels = torch.tensor(
                doc_fea.event_type_labels, device=device, dtype=torch.long, requires_grad=False
            )  # [num_event_types]
            doc_event_sent_loss = F.nll_loss(doc_event_sent_logps, doc_event_labels, reduction='sum')
            doc_event_prompt_loss = F.nll_loss(doc_event_prompt_logps, doc_event_labels, reduction='sum')
            doc_event_atten_loss = F.nll_loss(doc_event_sent_prompt_logps, doc_event_labels, reduction='sum')

            boot_pred_loss = F.nll_loss(boot_pred_logp, doc_event_labels, reduction='sum')


            return doc_event_sent_loss + doc_event_atten_loss + doc_event_prompt_loss + boot_pred_loss
        else:
            doc_event_pred_list = boot_pred_logp.argmax(dim=-1).tolist()
            return doc_event_pred_list
    ...
    def get_field_pred_logp(self, event_idx, field_idx, batch_span_emb, include_prob=False):
        event_table = self.event_tables2[event_idx]
    ...
    def conduct_field_level_reasoning(self, event_idx, field_idx, prev_decode_context, batch_span_context, role_seq_clss,
        ...
        field_query = role_seq_clss[event_idx][field_idx]
    ...
    def get_field_mle_loss_list(self, doc_sent_context, batch_span_context, event_idx,
                                field_idx2pre_path2cur_span_idx_set, role_seq_clss, structure_mask_2=None,
                                re_label=None, span_sent_id_list=None):
        field_mle_loss_list = []
        num_fields = self.event_tables2[event_idx].num_fields
        num_spans = batch_span_context.size(0)
        ...
        for field_idx in range(num_fields):
            ...
            for prev_path, cur_span_idx_set in prev_path2cur_span_idx_set.items():
                ...
                batch_cand_emb, prev_decode_context = self.conduct_field_level_reasoning(
                    event_idx, field_idx, prev_decode_context, batch_span_context, role_seq_clss, structure_mask_2, re_label,
                ...
                for span_idx in cur_span_idx_set:
                    ...
                    if span_idx is None:
                        span_context = self.event_tables2[event_idx].field_queries[field_idx]
                        span_context = role_seq_clss[event_idx][field_idx]
    ...
    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info, re_span_rep, prompt_sen_emb_list, doc_prompt_sent_list, alpha_event_list, entity_seq_cls,
                        role_seq_clss,
        ...
        event_cls_loss = self.get_event_cls_info2(doc_token_emb, doc_sent_context, doc_fea, prompt_sen_emb_list, doc_prompt_sent_list, entity_seq_cls,
                        role_seq_clss, train_flag=True)
        ...
        for event_idx, event_label in enumerate(doc_fea.event_type_labels):
            if event_label == 0:
                ...
                for field_idx in range(num_fields):
                    ...
                    # update the memory tensor替换有意义的表示
                ...
                field_loss_list = self.get_field_mle_loss_list(
                    doc_sent_context, batch_span_context, event_idx, field_idx2pre_path2cur_span_idx_set, role_seq_clss,
    ...
    def get_eval_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info, re_span_rep, prompt_sen_emb_list, doc_prompt_sent_list, entity_seq_cls,
                        role_seq_clss, re_pair_cands=None,
                        structure_mask=None, structure_mask_2=None, re_label=None, span_sent_id_list=None):
        span_context_list, doc_sent_context = self.get_doc_span_sent_context(
            doc_token_emb, doc_sent_emb, doc_fea, doc_span_info, re_span_rep, structure_mask=structure_mask)
        if len(span_context_list) == 0:
        ...
        batch_span_context = torch.cat(span_context_list, dim=0)
        #doc_token_emb, doc_sent_context, doc_fea, prompt_sen_emb_list, entity_seq_cls, role_seq_clss, train_flag = True
        # 1. get event type prediction
        event_pred_list = self.get_event_cls_info2(doc_token_emb, doc_sent_context, doc_fea, prompt_sen_emb_list, doc_prompt_sent_list, entity_seq_cls, role_seq_clss, train_flag=False)
        ...
        for event_idx, event_pred in enumerate(event_pred_list):
            ...
            for field_idx in range(num_fields):
                ...
                for prev_path in last_field_paths:  # traverse all previous decoding paths
                    ...
                    # conduct reasoning on this field
                    batch_cand_emb, prev_decode_context = self.conduct_field_level_reasoning(
                        event_idx, field_idx, prev_decode_context, batch_span_context, role_seq_clss, structure_mask_2,
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        batch_size = len(doc_batch_dict['ex_idx'])#例子的id号
        ...
        # [ner_batch_size, norm_sent_len]多个句子组合做序列标注
        ...
        if use_gold_span:  # definitely use gold span info#是否使用真实的跨度标签
            ner_token_types = ner_token_labels
        else:
            ner_token_types = ner_token_preds

        # get sentence embedding (consider pos encoding in the following step.)包含了位置信息的句子编码
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        if train_flag and self.config.use_scheduled_sampling:
            # teacher_prob will gradually decrease outside随机使用真实的span，随着训练的过程，使用真实span的概率会减小
            if random.random() < teacher_prob:
                use_gold_span = True
            else:
                use_gold_span = False
        ...
        # aaa = doc_batch_dict['doc_token_masks'].cpu().tolist()
        # bbb = doc_batch_dict['doc_token_masks'].cpu().tolist()
        # # a = torch.nonzero(doc_batch_dict['doc_token_ids'][0])
        # a = np.count_nonzero(aaa,axis=1)
        # b = np.count_nonzero(aaa,axis=1)
        # aa = np.min(a)
        # a1 = torch.nonzero(doc_batch_dict['doc_token_ids']).item()
        # b1 = torch.nonzero(doc_batch_dict['doc_token_masks']).item()
        prompt_token_ids_list = doc_batch_dict['prompt_token_ids']
        prompt_token_masks_list = doc_batch_dict['prompt_token_masks']
        valid_sent_num_list = doc_batch_dict['valid_sent_num']


        batch_size = len(doc_batch_dict['ex_idx'])#例子的id号
        prompt_batch_idx_start_list = [0]
        prompt_token_ids = []
        prompt_token_masks = []
        for batch_idx, valid_sent_num in enumerate(valid_sent_num_list):
            idx_start = prompt_batch_idx_start_list[-1]
            idx_end = idx_start + valid_sent_num
            prompt_batch_idx_start_list.append(idx_end)

            prompt_token_ids.append(prompt_token_ids_list[batch_idx])
            prompt_token_masks.append(prompt_token_masks_list[batch_idx])

        # [ner_batch_size, norm_sent_len]
        prompt_token_ids = torch.cat(prompt_token_ids, dim=0)
        prompt_token_masks = torch.cat(prompt_token_masks, dim=0)

        # get prompt output
        batch_seq_enc, prompt_sen_emb_list = self.ner_model.bert(prompt_token_ids,
                                     attention_mask=prompt_token_masks,
                                     output_all_encoded_layers=False)
        # [batch_size, seq_len, hidden_size]
        prompt_token_emb_list1 = self.ner_model.dropout(batch_seq_enc)
        prompt_sen_emb_list1 = prompt_sen_emb_list[0][:, 0, :]
        prompt_token_emb_list = []
        prompt_sen_emb_list = []
        for batch_idx in range(batch_size):
            idx_start = prompt_batch_idx_start_list[batch_idx]
            idx_end = prompt_batch_idx_start_list[batch_idx + 1]
            prompt_token_emb_list.append(prompt_token_emb_list1[idx_start:idx_end, :, :])
            prompt_sen_emb_list.append(prompt_sen_emb_list1[idx_start:idx_end, :])

        doc_token_emb = torch.cat(doc_token_emb_list, dim=0)#原始文本
        prompt_token_emb = torch.cat(prompt_token_emb_list, dim=0)#提示文本
        #prompt_sen_emb_list = torch.cat(prompt_sen_emb_list, dim=0)
        #inter_doc_prompt_emb = torch.cat([doc_token_emb, prompt_token_emb], dim=1)  # .unsqueeze(0)

        #doc_token_emb=Q,prompt_token_emb=K

        doc_token_emb = self.doc_prompt_context_encoder(doc_token_emb, prompt_token_emb, None,
                                                        structure_mask=None).squeeze(0)
        doc_sent_list1 = doc_token_emb[:, 0, :]
        doc_prompt_sent_list = []

        doc_token_emb_list1 = []#交互后的原文表示
        for batch_idx in range(batch_size):
            idx_start = prompt_batch_idx_start_list[batch_idx]
            idx_end = prompt_batch_idx_start_list[batch_idx + 1]
            doc_token_emb_list1.append(doc_token_emb[idx_start:idx_end, :, :])
            doc_prompt_sent_list.append(doc_sent_list1[idx_start:idx_end, :])

        #doc_prompt_context = inter_doc_prompt_emb[:, :len(doc_token_emb_list[0]), :]
        ...
        for k,v in self.entity_dict.items():
            input_token_ids.append(v['input_token_ids'])
            fea_masks.append(v['fea_masks'])
            # input_token_id = torch.tensor(v['input_token_ids'],device=prompt_token_ids.device)
            # fea_masks = torch.tensor(v['fea_masks'],device=prompt_token_ids.device)
            # #torch.tensor(v['input_token_ids'])
        input_token_ids = torch.tensor(input_token_ids, device=prompt_token_ids.device)
        fea_masks = torch.tensor(fea_masks, device=prompt_token_ids.device)
        seq_enc, entity_seq_cls = self.ner_model.bert(input_token_ids,
                                               attention_mask=fea_masks,
                                               output_all_encoded_layers=False)
        #pooling_cls = entity_seq_cls.pooler_output

        entity_seq_cls = entity_seq_cls[0][:, 0, :]
        entity_seq_cls = entity_seq_cls.unsqueeze(dim=1)#实体表示

        role_seq_clss=[]
        for id, (entity, files) in enumerate(self.event_type_fields_pairs):
            input_token_id=[]
            fea_mask = []
            for file in files:
                input_token_id.append(self.role_dict[entity+'-'+file]['input_token_ids'])
                fea_mask.append(self.role_dict[entity+'-'+file]['fea_masks'])
            input_token_ids = torch.tensor(input_token_id, device=prompt_token_ids.device)
            fea_masks = torch.tensor(fea_mask, device=prompt_token_ids.device)
            seq_enc, role_seq_cls = self.ner_model.bert(input_token_ids,
                                                   attention_mask=fea_masks,
                                                   output_all_encoded_layers=False)
            role_seq_clss.append(role_seq_cls[0][:, 0, :].unsqueeze(dim=1))#角色表示
        # a=role_seq_clss[3][1]
        # b=role_seq_clss[2][5]
        # c=role_seq_clss[3][1]
        ...
        #print('a')
        #filed
        # def approximate_argmax(logits, temperature=1.0):
        #     scaled_logits = logits / temperature
        #     softmax_probs = F.softmax(scaled_logits, dim=-1)
        #     argmax_approximation = torch.argmax(softmax_probs, dim=-1)
        #     return argmax_approximation

        # def softmax_with_temperature(logits, temperature=0.1):
        #     scaled_logits = logits / temperature
        #     softmax_probs = F.softmax(scaled_logits, dim=0)
        #     return softmax_probs
        # get doc feature objects
        ex_idx_list = doc_batch_dict['ex_idx']
        doc_fea_list = [doc_features[ex_idx] for ex_idx in ex_idx_list]
        alpha_sent_event_list = []
        alpha_prompt_event_list = []
        alpha_doc_event_list = []
        alpha_all_event_list = []
        ...
        for batch_idx, ex_idx in enumerate(ex_idx_list):
            ...
            alpha_doc_i=[]
            alpha_all_i=[]
            for event_idx, event_label in enumerate(doc_fea_list[batch_idx].event_type_labels):
                #原文
                sent_event_table = self.event_tables2[event_idx]
                event_query = entity_seq_cls[event_idx]
                _, alpha_sent = sent_event_table(query_context_emb=event_query,
                                                          sent_context_emb=doc_sent_emb_list[batch_idx])  # [1, hidden_size]
                alpha_sent_list.append(alpha_sent)
                #提示文本
                prompt_event_table = self.event_tables2[event_idx]
                _, atten_prompt = prompt_event_table(query_context_emb=event_query,
                                                                  sent_context_emb=prompt_sen_emb_list[batch_idx])
                alpha_prompt_list.append(atten_prompt)
                #交互文档
                doc_event_table = self.event_tables2[event_idx]
                _, atten_doc = doc_event_table(query_context_emb=event_query,
                                              sent_context_emb=doc_prompt_sent_list[batch_idx])
                alpha_doc_list.append(atten_doc)



            alpha_sent_list = torch.cat(alpha_sent_list, dim=0)
            alpha_prompt_list = torch.cat(alpha_prompt_list, dim=0)
            alpha_doc_list = torch.cat(alpha_doc_list, dim=0)
            k=4
            #a= alpha_sent_list.shape[1]
            if alpha_sent_list.shape[1]<k:
                k=alpha_sent_list.shape[1]
            _, alpha_sent_index = alpha_sent_list.topk(k, dim=1, largest=True, sorted=True)
            _, alpha_prompt_index = alpha_prompt_list.topk(k, dim=1, largest=True, sorted=True)
            _, alpha_doc_index = alpha_doc_list.topk(k, dim=1, largest=True, sorted=True)

            for i in alpha_sent_index.cpu().tolist():
                for j in i:
                    alpha_sent_i.append(j)
                    alpha_all_i.append(j)
            alpha_sent_i_set=set(alpha_sent_i)
            alpha_sent_event_list.append(list(alpha_sent_i_set))


            for i in alpha_prompt_index.cpu().tolist():
                for j in i:
                    alpha_prompt_i.append(j)
                    alpha_all_i.append(j)
            alpha_prompt_i_set=set(alpha_prompt_i)
            alpha_prompt_event_list.append(list(alpha_prompt_i_set))

            for i in alpha_doc_index.cpu().tolist():
                for j in i:
                    alpha_doc_i.append(j)
                    alpha_all_i.append(j)
            alpha_doc_i_set=set(alpha_doc_i)
            alpha_doc_event_list.append(list(alpha_doc_i_set))
            alpha_all_i_set=set(alpha_all_i)
            alpha_all_event_list.append(list(alpha_all_i_set))
        ...
        #         event_table = self.event_tables2[event_idx]
        #         event_query = entity_seq_cls[event_idx]
        #         cur_event_split_logp, alpha = event_table(query_context_emb=event_query,
        #                                                   sent_context_emb=prompt_sen_emb_list[
        #                                                       batch_idx])  # [1, hidden_size]
        #         alpha_list.append(alpha)
        #
        #     alpha_list = torch.cat(alpha_list, dim=0)
        #     alpha_event_list.append(alpha_list)
        #
        #     _, alpha_index = alpha_list.topk(5, dim=1, largest=True, sorted=True)
        #     for i in alpha_index.cpu().tolist():
        #         for j in i:
        #             alpha_i.append(j)
        #
        #     alpha_i_set = set(alpha_i)
        #     alpha_prompt_event_list.append(list(alpha_i_set))
        #
        # for batch_idx, ex_idx in enumerate(ex_idx_list):
        #     alpha_list = []
        #     alpha_i = []
        ...
        #
        #     alpha_i_set = set(alpha_i)
        #     alpha_sent_event_list.append(list(alpha_i_set))
        # get doc span-level info for event extraction
        doc_span_info_list = get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=use_gold_span, alpha_list= alpha_all_event_list)
            ...
            for batch_idx, ex_idx in enumerate(ex_idx_list):
                doc_event_loss_list.append(
                    self.get_loss_on_doc(
                        doc_token_emb_list1[batch_idx],
                        doc_sent_emb_list[batch_idx],
                        doc_fea_list[batch_idx],
                        doc_span_info_list[batch_idx],
                        re_span_rep_list[batch_idx],
                        prompt_sen_emb_list[batch_idx],
                        doc_prompt_sent_list[batch_idx],
                        alpha_all_event_list[batch_idx],
                        entity_seq_cls,
                        role_seq_clss,
...
class SentencePosEncoder(nn.Module):
    ...
    def forward(self, batch_elem_emb, sent_pos_ids=None):
        ...
        out = batch_elem_emb + batch_pos_emb#句子编码加位置编码
...
class EventTable2(nn.Module):
    def __init__(self, event_type, field_types, entity_dict, role_dict, hidden_size):
        super(EventTable2, self).__init__()
    ...
    def forward(self, query_context_emb=None, sent_context_emb=None, batch_span_emb=None, field_idx=None):
        assert (sent_context_emb is None) ^ (batch_span_emb is None)

        if sent_context_emb is not None:  # [num_spans+num_sents, hidden_size]
            # doc_emb.size = [1, hidden_size]
            doc_emb, alpha = transformer.attention(query_context_emb, sent_context_emb, sent_context_emb)
            #doc_emb, _ = transformer.attention(self.event_query, sent_context_emb, sent_context_emb)
            doc_pred_logits = self.event_cls(doc_emb)
            doc_pred_logp = F.log_softmax(doc_pred_logits, dim=-1)

            return doc_pred_logp, alpha
...
class EventTable3(nn.Module):
    def __init__(self, event_type, field_types, hidden_size):
        super(EventTable3, self).__init__()
    ...
    def forward(self, query_context_emb=None, sent_context_emb=None, batch_span_emb=None, field_idx=None):
        ...
        if sent_context_emb is not None:  # [num_spans+num_sents, hidden_size]
            # doc_emb.size = [1, hidden_size]
            #doc_emb, alpha = transformer.attention2(self.event_query, sent_context_emb, sent_context_emb)
            doc_emb, alpha = transformer.attention2(query_context_emb, sent_context_emb, sent_context_emb)
            doc_pred_logits = self.event_cls(doc_emb)
            doc_pred_logp = F.log_softmax(doc_pred_logits, dim=-1)
...
def get_span_mention_info(span_dranges_list, doc_token_type_list):
    ...
    #span_mention_range_list每个为一个重复出现多次的实体的位置， mention_drange_list所有的实体边界，包括重复的，mention_type_list提及的实体类别
