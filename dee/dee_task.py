...
def obtain():
    tokenizer = BERTChineseCharacterTokenizer.from_pretrained('model_base')

    fr = open('Data/chi_t_r_explain.json', 'r', encoding='utf-8')
    entity_r = {}
    role_r = {}
    for i in fr.readlines():
        a = json.loads(i)
        entity_r[a['type']['name_type']] = a['type']['explain_type']
        for j in range(len(a['role']['name_role'])):
            role_r[a['type']['name_type'] + '-' + a['role']['name_role'][j]] = a['role']['explain_role'][j]
    valid_token_len = 128
    valid_token_len -= 1
    valid_token_len -= 1
    for k, v in entity_r.items():
        prompt_text = tokenizer.char_tokenize(v)
        ...
        input_token_ids = tokenizer.convert_tokens_to_ids(fea_tokens)

        fea_seq_len = len(input_token_ids)
        fea_segment_ids = [0] * fea_seq_len
        fea_masks = [1] * fea_seq_len

        # feature is padded to max_seq_len, but fea_seq_len is the real length
        while len(input_token_ids) < 128:
            input_token_ids.append(0)
            fea_masks.append(0)
            fea_segment_ids.append(0)

        # token_id_mat.append(input_token_ids)
        # token_mask_mat.append(fea_masks)
        # segment_ids_mat.append(fea_segment_ids)
        entity_r[k] = {"input_token_ids": input_token_ids, "fea_masks": fea_masks, "fea_segment_ids": fea_segment_ids}

    for k, v in role_r.items():
        prompt_text = tokenizer.char_tokenize(v)
        # truncate according to max_seq_len and record some statistics
        if len(prompt_text) > valid_token_len:  # 文本和标签的截断处理
            prompt_text = prompt_text[:valid_token_len]
        ...
        # token_mask_mat.append(fea_masks)
        # segment_ids_mat.append(fea_segment_ids)
        role_r[k] = {"input_token_ids": input_token_ids, "fea_masks": fea_masks, "fea_segment_ids": fea_segment_ids}
    return entity_r, role_r

class DEETaskSetting(TaskSetting):
    base_key_attrs = TaskSetting.base_key_attrs
    entity_r, role_r=obtain()


    base_attr_default_pairs = [
        ('train_file_name', 'train_train_explain.json'),
        ('dev_file_name', 'dev_dev_explain.json'),
        ('test_file_name', 'test_test_explain.json'),
        ...
        ('bert_model', 'model_base'),  # use which pretrained bert model
        ...
        ('model_types', 'ReDEE'),  # decide the model class used
        ...
        ("entity_dict", entity_r),  # consider the number of relations.
        ("role_dict", role_r),  # consider the number of relations.
    ]
...
class DEETask(BasePytorchTask):
    ...
    def __init__(self, dee_setting, load_train=True, load_dev=True, load_test=True,
        ...
        self.setting.vocab_size = len(self.tokenizer.vocab.items())#赋值
        self.id_2_vocab = {pair[1]: pair[0] for pair in list(self.tokenizer.vocab.items())}

        # get entity and event label name (BIO format)
        self.entity_label_list = DEEExample.get_entity_label_list()#两种格式二选一
        ...
        if self.setting.model_types == 'ReDEE':
        ...
        elif self.setting.model_types == 'DCFEE':
